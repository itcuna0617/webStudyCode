...
//    }
    ...
    /*
        let 함수는 변수의 값이 null이 아닌 경우에 실행 할 코드 블럭을 작성할 수 있게 도와주는 함수
        it: value-parameter인 c가 null아 아닌 경우 c를 의미한다.
     */
//    c?.let {
...
//    }
    ...
    var two = 2 ?: 1     // 왼쪽 피연산자의 값이 null이 아니므로 왼쪽 피연산자값(2)를 반환
    println(two)

    /* 4. null값이 아님을 보증하는 연산자를 사용해 null을 허용하지 않는 타입으로 변환 */
    /*
        !! 연산자를 이용하여 타입을 null 불가능한 타입이라고 명명할 수는 있지만 컴파일 에러를
        런타임 Exception으로 바꾼 것 뿐이고 NPE는 그대로 발생한다.
     */
