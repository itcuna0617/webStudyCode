...
//    immutableList.add(4)        // 추가 불가
//    immutableList.remove(1)     // 제거 불가
    println("immutable list index [2]: ${immutableList[2]}")    // 읽는 것은 가능

    /* immutableList -> mutableList로 전환 가능 */
    var immutableToMutableList = immutableList.toMutableList()

    immutableToMutableList.add(4)
    immutableToMutableList.remove(4)

    /* + 연산자로 두 리스트를 합칠 수 있다.(동일한 타입만 가능) */
    var plusList: List<Char> = listOf('a', 'b', 'c') + listOf('d', 'e', 'f')
    println("list concat: $plusList")

    /* - 연산자로 앞의 리스트에서 뒤의 리스트의 내용을 삭제할 리스트를 얻을 수 있음(중복 제거 가능) */
    var substractList: List<Int> = listOf(1, 2, 3, 1, 3, 4, 5, 2, 6) - listOf(2, 4, 6, 1)
    println("list substract set: $substractList")
