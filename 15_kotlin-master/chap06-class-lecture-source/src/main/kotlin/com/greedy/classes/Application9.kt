...
/* 클래스의 내용({})을 쓰지 않아도 필드 두개 선언 및 getter/setter부터 toString까지 모두 정의 됨 */
data class PersonClass(var name: String, var age: Int)

fun main(args: Array<String>) {
    var p1 = PersonClass("홍길동", 17)
    var p2 = PersonClass("홍길동", 17)
    var p3 = PersonClass("유관순", 21)
    ...
    println(copied1 == p1)  // 사본이 동등한 값을 지녔는지 확인
    println(copied1 === p1) // 깊은 복사인지 얕은 복사인지 확인(깊은 복사)

    /* <componentN> */
    /*
        component 관련 메소드들은 쓸모없어 보이지만 구조분해 할당(Destructuring Declaration)을 이요할 때
        componentN 메소드가 유용하게 활용된다.
        (구조 분해 할당은 객체의 속성값을 간편하게 여러 개의 변수에 대입하기 위해 사용하는 방법이다.)
     */
