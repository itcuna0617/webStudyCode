...
/*
    ...
    - 변수를 선언할 때 초기화 코드도 함께 정의하고 변수가 사용될 때(최초)
      초기화 코드가 단 한번 동작하여 변수를 초기화 함
    
    lateinit 키워드(나중에 값이 들어올꺼니까 걱정하지 말라고 하는 것과 비슷하다.)
    - 필요한 시점에 해당 필드를 초기화 해서 사용할 수 있음
      객체를 만드는데는 문제없이 보장한다고는 하지만 초기화하지 않고 해당 필드를
      활용할 때 Exception이 발생할 수 있으므로 주의를 요함(초기화 잊으면 안됨)
 */
class LazyClass(var x: Int) {

    val lazyValue1 by lazy {
        println("lazy 람다 식 내부에서 속성 값 초기화 진행")
        var s = " HELLO "
        s.toLowerCase().trim()      // 이 값이 return되어 필드에 대입 됨
    }
    ...
    val costHeavyProperty by lazy {
        ...
        Array<Byte>(1024 * 1024 * 100) {0}  // 100MByte의 큰 배열 선언
    }
}
...
class LateInitClass {
    ...
    /*
        1. 처음에 아무값도 대입하지 않고 쓰기 위한 키워드이다.
        2. val은 사용이 불가능하다. (해당 필드의 setter를 써서 값을 넣어줘야 되기 때문에)
        3. lateinit 프로퍼티는 오직 클래스 타입(레퍼런스 타입)만 지원한다.(기본자료형 or String은 안됨)
     */
...
class PropertyObject {
    fun func() {
        println("from ProeprtyObject Function")
    }
}
