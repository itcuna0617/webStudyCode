...
<body>
	<h1 algin="center">뷰 리졸버를 이용한 뷰 연결하기</h1>
	
	<!-- 
		핸들러 메소드가 요청을 처리하고 뷰 이름을 반환하면 DispatcherServlet은 화면에서 데이터를 표시하도록
		뷰 템플릿에 제어권을 넘긴다.
		스프링 MVC에서는 다양한 전략에 맞게 뷰를 해석할 수 있는 ViewResolver 구현체 몇 가지가 있다.
		그 중 MVC 기본 설정에는 템플릿 명과 위치에 따른 뷰를 해석하는 InternalResourceViewResolver를
		기본으로 사용하고 있다.
		prefix/suffix를 이용해 뷰 이름을 특정 애플리케이션 디렉토리에 대응 시킨다.
		InternalResourceViewResolver는 사용이 간단해서 좋기는 하지만 RequestDispatcher가 forward할
		수 있는 내부 리소스(jsp 또는 서블릿)만 해석이 가능하기 때문에, 다른 뷰 템플릿을 사용하는 경우에는
		다른 viewResolver를 사용해야 한다.
	 -->
	 ...
	 <script>
	 	...
	 	console.log(message);
	 	if(message) {			/* 이 페이지가 렌더링 될때 넘겨진 parameter가 있어서 빈 문자열이 아니면 */
	 		alert(message);
	 	}
	 </script>
	 
	 <h3>문자열로 redirect하기(flashAttr 사용하기)</h3>
	 ...
	 <button onclick="location.href='mv-redirect'">ModelAndView로 뷰 이름 반환하며 리다이렉트</button>
	 <script>
	 ...
	 <h3>ModelAndView로 redirect하기(flashAttr 사용하기)</h3>
	 <button onclick="location.href='mv-redirect-attr'">
